import { characterDetails } from "@/app/lib/utils/api";
        ...
        <>
            <div className="flex flex-col items-center overflow-auto h-screen p-4 md:p-8">
                ...
                <div className="flex flex-col items-center space-y-4">
                    {character.name && <h2 className="text-2xl font-semibold">Name: {character.name}</h2>}
                    <Image src='/individualSW.jpeg' alt='Generic'
                        height={250}
                        width={250}
                        className="rounded-full"
                    />
                    {character.eye_color && <h2 className="text-lg">Eye color: {character.eye_color}</h2>}
                    {character.birth_year && <h2 className="text-lg">Birth Year: {character.birth_year}</h2>}
